...
export function MemberList(){
  const [users, setUsers] = useState(
    [
      {id : 1, name : '홍길동', age : 34, job : '혁명가'},
      {id : 2, name : '이순신', age : 37, job : '군인'},
      {id : 3, name : '세종대왕님', age : 84, job : '정치인'},
    ]
  )
    // 텍스트값용 변수
  ...
  const addUser = (e) => {
    ...
    const newUser = {id : users.length+1, name : name, age : Number(age), job : job}
      ...
      {users.map(
        (user) => (
          <li key = {user.id}>
            ...
            <button class="delBtn" onClick = {() => deleteUser(user.id)}>삭제
            </button></li>
          )
        )
      }
